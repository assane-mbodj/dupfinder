...
process GENE_DUPLICATED {
	...
	tuple val(prefix), path("${prefix}.gene_duplicated.bed"), emit: duplicated_gene_ch
	...
	bedtools intersect -a ${annotation} -b ${prefix}.merged.DUP_survivor.bed -f 0.25 -wa| bedtools sort | uniq > ${prefix}.gene_duplicated.bed

	
	bedtools intersect -a ${annotation} -b ${prefix}.merged.DUP_survivor.bed -f 0.25 -wa -wb|uniq > ${prefix}.intersectBed.csv
